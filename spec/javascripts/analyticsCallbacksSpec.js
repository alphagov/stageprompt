describe("Google Analytics callback", function () {
  ...
  it("should push an event onto the google analytics que", function () {
    ...
    expect(_gaq.push.argsForCall[0][0]).toEqual('_trackEvent');
  });
  ...
  it("should use sensible default values... eg. non interaction events so as not to mess with bounce rate", function () {
    ...
    var category = _gaq.push.argsForCall[0][1],
        action = _gaq.push.argsForCall[0][2],
        label = _gaq.push.argsForCall[0][3],
        value = _gaq.push.argsForCall[0][4],
        nonInteraction = _gaq.push.argsForCall[0][5];
