describe("stageprompt", function () {
  ...
  describe("journey events from landing on a page", function () {
    ...
    it("should send an event if the page has a journey-data tag on another tag", function () {
      ...
      expect(spy).toHaveBeenCalledWith("test-journey:nextStep");
    });
    ...
    it("should send one event if the page has multiple elements with data-journey attribute", function () {
      ...
      document.getElementById('sandbox').innerHTML = "<p id='foo' data-journey='test-journey:moreStuff'>something</p>";
      var spy = jasmine.createSpy();
      GOVUK.performance.stageprompt.setup({
        analyticsFunction:spy
      });
